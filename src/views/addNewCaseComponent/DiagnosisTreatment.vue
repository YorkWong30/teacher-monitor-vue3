          ...
          <template v-if="task.status !== 'done'">
            <div class="task-form">
              ...
              <div class="task-row row">
                ...
                <van-field v-model="task.description" type="textarea" :autosize="{ minHeight: 60, maxHeight: 200 }"
                  placeholder="请输入任务描述：" class="card-textarea" />
              </div>

              <div class="task-row row">
                <div class="task-label" style="margin-bottom: 5px;">任务结果：</div>
                <van-field v-model="task.result" type="textarea" :autosize="{ minHeight: 60, maxHeight: 200 }"
                  placeholder="请输入任务结果" class="card-textarea" />
              </div>
              <div class="task-actions x-f">
                <van-button size="mini" class="btn-cancel" @click="removeTask(index)">删除</van-button>
                <!-- <van-button size="mini" type="primary" class="btn-ok" @click="confirmTask(index)">保存</van-button> -->
              </div>
            </div>
          </template>
          <template v-else>
            <div class="task-read">
              <div class="task-read-row"><span class="task-read-label">任务名称：</span><span class="task-read-text">{{
                task.name || '-' }}</span></div>
